...
Fits `plays.fit_clusters` on the `split == "train"` rows, assigns every row a cluster, writes
`clusters.json`, and renders a 3x3 grid of setup tiles (the nine nearest members) per cluster to
...
def main() -> None:
    ...
    model = P.fit_clusters(X[train_idx], k_range=range(args.k_min, args.k_max + 1))
    labels = P.assign(model, X)
    clusters = P.summarize(model, rows, labels, Z.ZONE_NAMES)
    ...
    for c in clusters:
        print(f"cluster {c['cluster']}: n={c['n']} by_bucket={c['n_by_bucket']} "
              f"by_split={c['n_by_split']}")
    ...
    out = {
        ...
        "clusters": clusters,
        "labels": {f"{r.game_id}:{r.period}:{r.index}": int(labels[i])
                   for i, r in enumerate(rows)},
    }
